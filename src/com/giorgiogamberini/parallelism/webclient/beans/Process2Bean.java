...
@SessionScoped
@Stateless
public class Process2Bean implements Serializable {
	...
	@Asynchronous
	public void runSchedule( @Observes Process2Bean process2Bean ) {
		this.scheduleProcess2();
	}

	public String scheduleProcess2() {
		...
		try {
			...
			mapObject.put( "keyString", "Test-2" );
			mapObject.put( "keyObject", process );
			objMsg.setObject( (Serializable) mapObject );

			sender.send( objMsg );

			System.out.println( "Sent Process2 to queue.." );
			...
			//			System.out.println( "* Scheduled MDSP 2: " + process.getDesiredTime() + "s" );
